...
- **API REST** (Node/Express + Supabase) en la carpeta `api/`: login, CRUD de cuerpos, hermanos, grupos, otros orientes (misma lógica que el sistema PHP viejo).
...
En otra terminal, desde la carpeta `api/`:
...
- **`api/`** – Backend Node/Express con **Supabase** (auth + base de datos). Ver `api/README.md`.
- **`PLAN-SCG33.md`** – Análisis del sistema viejo y plan del nuevo (admin + miembros + web).

## Conectar la app con la API

1. **Levantá la API** (ver `api/README.md`): en la carpeta `api/` ejecutá `npm start` (o `npm run dev`).
...
Comprobá que la API esté corriendo: `cd api && npm run dev`. Deberías ver algo como `SCG33 API escuchando en http://localhost:4000`. Revisá `EXPO_PUBLIC_API_URL` en `.env` si usás otra URL.
